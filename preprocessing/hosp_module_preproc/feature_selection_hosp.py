...
def feature_nonicu(
    ...
    if diag_flag:
        ...
        diag[
            ["subject_id", "hadm_id", "icd_code", "root_icd10_convert", "root"]
        ].to_csv("./data/features/preproc_diag.csv.gz", compression="gzip", index=False)
    ...
    if proc_flag:
        ...
        proc = preproc_proc(
            "./" + version_path + "/hosp/procedures_icd.csv.gz",
            "./data/cohort/" + cohort_output + ".csv.gz",
            "chartdate",
            "base_anchor_year",
            dtypes=None,
        ...
        ].to_csv("./data/features/preproc_proc.csv.gz", compression="gzip", index=False)
        print("[SUCCESSFULLY SAVED PROCEDURES DATA]")

    if med_flag:
        print("[EXTRACTING MEDICATIONS DATA]")
        med = preproc_meds(
            "./" + version_path + "/hosp/prescriptions.csv.gz",
            "./data/cohort/" + cohort_output + ".csv.gz",
        ...
        ].to_csv("./data/features/preproc_med.csv.gz", compression="gzip", index=False)
        print("[SUCCESSFULLY SAVED MEDICATIONS DATA]")

    if lab_flag:
        print("[EXTRACTING LABS DATA]")
        lab = preproc_labs(
            "./" + version_path + "/hosp/labevents.csv.gz",
            version_path,
            "./data/cohort/" + cohort_output + ".csv.gz",
            "charttime",
            "base_anchor_year",
        ...
        ].to_csv("./data/features/preproc_labs.csv.gz", compression="gzip", index=False)
