...
def read_labevents_table(mimic4_path):
    labevents = dataframe_from_csv(
        os.path.join(mimic4_path, "hosp/labevents.csv.gz"), chunksize=1000
    )
...
def read_d_labitems_table(mimic4_path):
    labitems = dataframe_from_csv(
        os.path.join(mimic4_path, "hosp/d_labitems.csv.gz"), chunksize=1000
    )
...
########################## PREPROCESSING ##########################
def get_range(
    df: pd.DataFrame, time_col: str, anchor_col: str, measure="days"
) -> pd.Series:
    ...
    if measure == "years":
        ...
        return pd.Series(
            list(zip(df.min_year_group + shift, df.max_year_group + shift))
        ).apply(get_year_timedelta)
    elif measure == "days":
...
def preproc_meds(module_path: str, adm_cohort_path: str, mapping: str) -> pd.DataFrame:
    adm = pd.read_csv(
        adm_cohort_path, usecols=["hadm_id", "admittime"], parse_dates=["admittime"]
    )
    med = pd.read_csv(
        module_path,
        compression="gzip",
...
def preproc_labs(
    ...
    if version_path == "mimiciv/1.0":
        adm = pd.read_csv(
            "./" + version_path + "/core/admissions.csv.gz",
            header=0,
            index_col=None,
            compression="gzip",
        ...
        adm = pd.read_csv(
            "./" + version_path + "/hosp/admissions.csv.gz",
            header=0,
            index_col=None,
            compression="gzip",
            usecols=["subject_id", "hadm_id", "admittime", "dischtime"],
