...
def fairness_evaluation(inputFile, outputFile):
    ...
    output_dict["Predicted"] = output_dict.apply(
        lambda row: 1 if row.Prob >= 0.5 else 0, axis=1
    )
    ...
    for sens_col in sensitive_columns:
        for group, aggregate in output_dict.groupby(sens_col):
            ...
            tp, tn, fp, fn, tpr, tnr, fpr, fnr, pr, nr, acc = get_cm_parameters(
                list(aggregate.Labels), list(aggregate.Predicted)
            )
            tmp_dct.update(
                dict(
                    group=group,
