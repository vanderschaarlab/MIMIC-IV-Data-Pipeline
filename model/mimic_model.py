...
class LSTMBase(nn.Module):
    ...
    def build(self):
        if self.med_vocab_size:
            self.med = ValEmbed(
                self.device, self.med_vocab_size, self.embed_size, self.latent_size
            )
        if self.proc_vocab_size:
            self.proc = CodeEmbed(
                self.device, self.proc_vocab_size, self.embed_size, self.latent_size
            )
        if self.out_vocab_size:
            self.out = CodeEmbed(
                self.device, self.out_vocab_size, self.embed_size, self.latent_size
            )
        if self.chart_vocab_size:
            self.chart = ValEmbed(
                self.device, self.chart_vocab_size, self.embed_size, self.latent_size
            )
        if self.lab_vocab_size:
            self.lab = ValEmbed(
                self.device, self.lab_vocab_size, self.embed_size, self.latent_size
            )

        if self.cond_vocab_size:
            self.cond = StatEmbed(
                self.device, self.cond_vocab_size, self.embed_size, self.latent_size
            )

        self.ethEmbed = nn.Embedding(
            self.eth_vocab_size, self.latent_size, self.padding_idx
        )
        self.genderEmbed = nn.Embedding(
            self.gender_vocab_size, self.latent_size, self.padding_idx
        )
        self.ageEmbed = nn.Embedding(
            self.age_vocab_size, self.latent_size, self.padding_idx
        )
        self.insEmbed = nn.Embedding(
            self.ins_vocab_size, self.latent_size, self.padding_idx
        )

        self.embedfc = nn.Linear(
            (self.latent_size * (self.modalities + 4)), self.latent_size, True
        )
        self.rnn = nn.LSTM(
            input_size=self.latent_size,
            hidden_size=self.rnn_size,
            num_layers=args.rnnLayers,
            batch_first=True,
        )
...
class LSTMBaseH(nn.Module):
    ...
    def build(self):
        ...
        self.embedfc = nn.Linear(
            (self.latent_size * (self.modalities - 1)), self.latent_size, True
        )
...
class StatEmbed(nn.Module):
    ...
    def build(self):
        ...
        self.fc = nn.Linear(
            self.embed_size * self.code_vocab_size, self.latent_size, True
        )
...
class CodeEmbed(nn.Module):
    ...
    def forward(self, code):
        ...
        codeEmbedded = torch.mul(code, codeEmbedded)
        codeEmbedded = torch.reshape(
            codeEmbedded, (codeEmbedded.shape[0], codeEmbedded.shape[1], -1)
        )
        codeEmbedded = codeEmbedded.to(self.device)
        #         print(codeEmbedded.shape)
...
class LSTMAttn(nn.Module):
    ...
    def build(self):
        ...
        self.condEmbed = nn.Embedding(
            self.cond_vocab_size, self.embed_size, self.padding_idx
        )
        self.condfc = nn.Linear(
            (self.embed_size * self.cond_seq_len), self.rnn_size, False
        )

        self.ethEmbed = nn.Embedding(
            self.eth_vocab_size, self.embed_size, self.padding_idx
        )
        self.genderEmbed = nn.Embedding(
            self.gender_vocab_size, self.embed_size, self.padding_idx
        )
        self.ageEmbed = nn.Embedding(
            self.age_vocab_size, self.embed_size, self.padding_idx
        )
        ...
        self.fc1 = nn.Linear(
            int(self.rnn_size * (self.modalities + 2)),
            int((self.rnn_size * (self.modalities + 2)) / 2),
            False,
        )
        self.fc2 = nn.Linear(
            int((self.rnn_size * (self.modalities + 2)) / 2),
            int((self.rnn_size * (self.modalities + 2)) / 4),
            False,
        )
...
class CodeAttn(nn.Module):
    ...
    def build(self):
        self.codeEmbed = nn.Embedding(
            self.code_vocab_size, self.embed_size, self.padding_idx
        )
        if self.signal:
            self.codeRnn = nn.LSTM(
                input_size=int(self.embed_size * self.code_seq_len),
                hidden_size=self.rnn_size,
                num_layers=2,
                dropout=0.2,
                batch_first=True,
            ...
            self.codeRnn = nn.LSTM(
                input_size=int((self.embed_size + 1) * self.code_seq_len),
                hidden_size=self.rnn_size,
                num_layers=2,
                dropout=0.2,
                batch_first=True,
...
class CNNBase(nn.Module):
    ...
    def build(self):
        ...
        self.conv1 = nn.Conv1d(
            self.latent_size, self.rnn_size, kernel_size=10, stride=1, padding=0
        )
