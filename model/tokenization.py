...
class BEHRT_models:
    def __init__(
        self, data_icu, diag_flag, proc_flag, out_flag, chart_flag, med_flag, lab_flag
    ):
    ...
    def tokenize_dataset(
        ...
        for patient, group in tqdm.tqdm(labs_input.groupby(self.id)):
            ...
            for row in cond_input[cond_input[self.id] == patient].itertuples(
                index=None
            ):
            ...
            if len(tokenized_src[idx]) >= 512:
                ...
                gender = gender_vocab[
                    demo_input[demo_input[self.id] == patient].iloc[0, 1]
                ]
                ethnicity = demo_vocab[
                    demo_input[demo_input[self.id] == patient].iloc[0, 2]
                ]
                insurance = ins_vocab[
                    demo_input[demo_input[self.id] == patient].iloc[0, 3]
                ]
        ...
        return (
            pd.DataFrame(tokenized_src),
            pd.DataFrame(tokenized_gender),
            pd.DataFrame(tokenized_ethni),
            pd.DataFrame(tokenized_ins),
            pd.DataFrame(tokenized_age),
            pd.DataFrame(tokenized_labels),
        )
    ...
    def tokenize(self):
        ...
        for col in labs_list.columns.to_list()[:-1]:
            ...
            if labs_list[col].nunique() > 1:
                for i in range(
                    len(pd.qcut(labs_list[col], 4, duplicates="drop", retbins=True)[1])
                    - 1
                ):
                    labels_l.append(str(col) + "_" + str(i))
                labs_codes.update(labels_l)
                labs_list[col] = pd.qcut(
                    labs_list[col], 4, labels=labels_l, duplicates="drop"
                )
            elif labs_list[col].nunique() == 1:
                labs_list.loc[
                    labs_list[labs_list[col] > 0][col].index, col
                ] = "dyn_" + str(col)
        ...
        print(
            str(len(tokenized_labels[tokenized_labels[0] == 1])) + " Positive samples."
        )
        print(
            str(len(tokenized_labels[tokenized_labels[0] == 0]))
            + " Negative samples.\n"
        )
        ...
        for i in range(len(ethVocab_reversed)):
            print(
                str(len(tokenized_ethni[tokenized_ethni[0] == i]))
                + " "
                + ethVocab_reversed[i]
                + " samples."
            )
        ...
        for i in range(len(insVocab_reversed)):
            print(
                str(len(tokenized_ins[tokenized_ins[0] == i]))
                + " "
                + insVocab_reversed[i]
                + " samples."
            )
